...
/*   Updated: 2025/10/13 18:18:05 by mstepnic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	extend_env(t_data *data, char **token_value, int is_in_quotes, int *i)
{
	...
	if (!env_name)
		return (EXIT_FAILURE);
	env_value = ft_getenv(data->env_list, env_name);
	if (!env_value)
		env_value = ft_strdup("");
	if (!env_value)
		return (free(env_name), EXIT_FAILURE);
...
int	handle_envs(t_data *data, t_token *tokens)
{
	...
	while (tokens)
	{
		...
		if (value && value[1] && !ft_strchr("\'\"<>|()& ", value[1]))
		{
